  ...
  return (
    <div className="font-grotesk">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold font-exo2">Event Schedule</h2>
        <div className="flex space-x-4">
          <button 
            onClick={() => setSelectedDay("March 1")}
            className={`px-4 py-2 rounded ${selectedDay === "March 1" 
          ...
          <button 
            ...
            className={`px-4 py-2 rounded ${selectedDay === "March 2" 
              ? "bg-[#c593e9] text-white" 
              : "bg-black/20 text-white/80"}`}
          >
            Day 2
        ...
        {dayEvents.map((event) => (
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <p className="text-sm bg-black/30 px-2 py-1 rounded"> 📍 {event.location}</p>
            </div>
            <h3 className="text-xl font-bold mt-1">{event.title}</h3>
            {event.description && <p className="mt-2 text-white/80">{event.description}</p>}
          </div>
        ))}
      </div>
    </div>
